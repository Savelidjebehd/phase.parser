...
async def admin_src_add_cb(call: CallbackQuery):
    ...
    if uid in _src_picker and _src_picker[uid].get("chats"):
        await _draw_picker(call, uid)
        return

    await safe_edit(call, "⏳ <b>Загружаю список чатов...</b>", None)

    # Загружаем диалоги один раз и кэшируем
    try:
        chats = []
        async for dialog in _userbot.iter_dialogs(limit=200):
            e = dialog.entity
        ...
        await safe_edit(call, f"❌ Ошибка загрузки чатов:\n<code>{ex}</code>", kb_back("admin_sources"))
        return
    ...
    if not chats:
        await safe_edit(call,
            ...
            "Вступите в нужные группы от имени UserBot и повторите.",
...
# _src_picker: uid -> {"chats": {str_id: {...}}, "selected": set(str_id)}
_src_picker: dict[int, dict] = {}


async def _draw_picker(call: CallbackQuery, uid: int) -> None:
    """Перерисовывает список без повторной загрузки диалогов."""
    state    = _src_picker.get(uid)
    if not state:
        await safe_edit(call, "❌ Сессия устарела. Начните заново.", kb_back("admin_sources"))
        return

    chats    = state["chats"]           # dict str_id -> chat
    selected = state["selected"]        # set of str_id

    existing_ids = {str(s["chat_id"]) for s in _db.get_sources(active_only=False)}

    rows = []
    for cid, c in chats.items():
        in_db   = cid in existing_ids
        checked = cid in selected
        if in_db:
            icon = "✅"
            cd   = "noop"
            suffix = " <i>(уже добавлен)</i>"
        elif checked:
            icon = "☑️"
            cd   = f"src_pick_toggle:{cid}"
            suffix = ""
        else:
            icon = "◻️"
            cd   = f"src_pick_toggle:{cid}"
            suffix = ""
        label = f"{icon} {c['title'][:38]}{suffix}"
        rows.append([(label, cd)])

    sel_count = len(selected)
    rows.append([(f"✅ Готово — добавить {sel_count}", "src_pick_done")])
    rows.append([("🔄 Обновить список", "src_pick_reload"), ("◀️ Назад", "admin_sources")])

    await safe_edit(call,
        f"<b>📡 Добавление источников</b>\n\n"
        f"Нажмите на группу чтобы выбрать (☑️) или снять (◻️).\n"
...
async def src_pick_done_cb(call: CallbackQuery):
    ...
    await safe_edit(call, "\n".join(parts), kb_back("admin_sources"))
